...
class PaymentOutWidget extends StatefulWidget {
  ...
  const PaymentOutWidget({Key? key,this.data,this.edit,this.view}) : super(key: key);

  @override
  _PaymentOutWidgetState createState() {
    return _PaymentOutWidgetState();
  }
}
...
class _PaymentOutWidgetState extends State<PaymentOutWidget> {
  ...
  TextEditingController _paymentAmount = TextEditingController();
  var _customerName = "";
  var user,admin;
  int state = 0;
  ...
  fetchPartyList(){
    ...
    FirebaseFirestore.instance.collection('party').get().then((party) {
      party.docs.forEach((doc) {
        partylist.add(doc.id);
      });
    });

    setState(() {
      _partyNames = partylist;
    });
  }
  ...
  firebaseUpdateHandler(data, admin, user) {
    ...
    if (needtoUpdate) {
      ...
      FirebaseFirestore.instance.collection('log').add({
        ...
        'time': DateTime.now(),
        'data':
            "Company: ${data['company']} | Amount: ₹${NumberFormat("##,##,#00.00").format(double.parse(data['amount'].toString()))} Changed Data: $changedData",
        'notSeen': users
      });
      FirebaseFirestore.instance
          .collection('party')
  ...
  handleFirebase(data,user,admin){
    ...
    FirebaseFirestore.instance.collection('paymentout').add(data).then((doc){
      FirebaseFirestore.instance.collection('log').add({
        'title': "Payment Out",
        'subTitle': data['customerName'],
        'time': DateTime.now(),
        'data': "Amount: ₹${NumberFormat("##,##,#00.00").format(double.parse(data['amount'].toString()))} | Company: ${data['company']}",
        'generatedBy': user,
        'approved': admin,
        'notSeen': users,

      });
      var partyRef = FirebaseFirestore.instance.collection('party').doc(data['customerName']);
      partyRef.get().then((party){
        ...
        showDialog(context: context, builder: (context) => AlertDialog(
          title: Text("Paid ₹${NumberFormat("##,##,#00.00").format(double.parse(data['amount'].toString()))} to ${data['customerName']}"),
          actions: [
  ...
  // Future getImages(media) async {
  //   var image = await _imagePicker.getImage(source: media);
  //   setState(() {
  //     images.add(image!);
  //   });
  // }
          ...
          child: RawAutocomplete(
            ...
            fieldViewBuilder:
              ...
              return Container(
                ...
                child: TextFormField(
                  readOnly: widget.view || widget.edit,
            ...
            onSelected:widget.view || widget.edit
                    ? null
                    :  (String? option) {
              setState(() {
